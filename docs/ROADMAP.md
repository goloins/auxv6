...
| USB subsystem | 50% | Initial PCI USB host-controller scaffold is in-tree (`UHCI/OHCI/EHCI/xHCI` classification, per-controller `init/ready/degraded` phase tracking, conservative BAR0 register sanity probes, `/proc/usb` observability, split HCD backends behind core ops dispatch, and backend lifecycle hooks for `probe/reset/halt/start` with explicit error-state reporting). All four backends have real guarded control paths (EHCI/xHCI MMIO `USBCMD`/`USBSTS` sequences; OHCI `HcCommandStatus.HCR` + `HcControl.HCFS` field; UHCI I/O-space `inw`/`outw` RS+HCH). After `start`, all backends now run a `scan_ports` pass that reads per-port PORTSC registers (EHCI: opbase+0x44+n*4; xHCI: opbase+0x400+n*0x10; OHCI: HcRhPortStatus at 0x54+n*4; UHCI: iobase+0x10+n*2) and populates `rh_connect_bits` (CCS per port) and `rh_change_bits` (CSC per port). Per-controller lifecycle and scan attempt/success/failure counters are tracked in-state and emitted via `/proc/usb`. Transfer rings/schedules, full root-hub emulation, endpoint state machine, and class drivers remain follow-on |
