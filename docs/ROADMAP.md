...
| Graphics / framebuffer console | 60% | Framebuffer core, display registry, builtin font/render path, `/proc/gfxstats`, virtio-gpu scanout discovery, display-sized console framebuffer allocation, and initial modern virtio-pci gpu transport support are landed; the normal console path is still CGA-authoritative, the tty grid is still fixed 80x25, and no `/dev/fb0` or `/dev/dri/card0` userspace ABI exists yet |
