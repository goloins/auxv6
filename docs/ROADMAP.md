...
| Modern storage | 91% | AHCI now has interrupt-driven completions, slot allocation, telemetry, fault-injection hooks, and explicit NVIDIA MCP79 SATA (10de:0ab5) profile wiring in the shared AHCI path; NVMe correctness hardening complete (polled-only IRQ model, monotonic CID counter, recovery memory-safety, shutdown notification, LBA-size guard), dev-number collision with loop devices fixed, and ext2/ext2fs mount alias validated on `/dev/nda`; NVMe timeout/reset recovery path is in place |
...
| USB subsystem | 65% | Initial PCI USB host-controller scaffold is in-tree (`UHCI/OHCI/EHCI/xHCI` classification, per-controller `init/ready/degraded` phase tracking, conservative BAR0 register sanity probes, `/proc/usb` observability, split HCD backends behind core ops dispatch, and backend lifecycle hooks for `probe/reset/halt/start` with explicit error-state reporting). All four backends have real guarded control paths (EHCI/xHCI MMIO `USBCMD`/`USBSTS` sequences; OHCI `HcCommandStatus.HCR` + `HcControl.HCFS` field; UHCI I/O-space `inw`/`outw` RS+HCH). After `start`, all backends run `scan_ports` (per-port CCS/CSC snapshot) plus `service_ports` reset/enable sequencing for connected ports (EHCI `PORTSC.PR/PED`, xHCI `PORTSC.PR/PED`, OHCI `HcRhPortStatus.PRS/PES`, UHCI `PORTSC.PR/PED`) with bounded delays/polling. Core USB now synthesizes a first-pass per-port candidate-device table (`usb_dev_count` + `devN` rows with `hc/kind/port/present/enabled/speed/addr_ready`) derived from root-hub state, and candidate `speed` is now decoded from backend port state (`rh_low/rh_full/rh_high/rh_super` masks: UHCI/OHCI LSDA, EHCI enabled high-speed, xHCI Port Speed ID). `/proc/usb` exports lifecycle + scan + service telemetry (`p/r/h/s/sc/sv`), connectivity/enabled masks, and speed masks, with a 4096-byte budget to avoid truncation as controller/device lines grow. Transfer rings/schedules, full root-hub emulation, endpoint state machine, and class drivers remain follow-on |
