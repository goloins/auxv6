...
| Graphics / framebuffer console | 58% | Framebuffer core, display registry, builtin font/render path, `/proc/gfxstats`, virtio-gpu scanout discovery, and display-sized console framebuffer allocation are landed; the normal console path is still CGA-authoritative, the tty grid is still fixed 80x25, and no `/dev/fb0` or `/dev/dri/card0` userspace ABI exists yet |
