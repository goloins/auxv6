...
- **AHCI recovery and endurance automation tranche landed 2026-04-02:** `ahci_submit_rw()` now treats pre-command port-idle stalls as first-class recoverable failures, feeding the same timeout accounting and `ahci_port_recover()` path used for command timeouts/taskfile errors instead of failing immediately. Guest automation now includes a dedicated AHCI endurance path with `ahci-stress.img`, `qemu-nox-ahcistress`, `tools/tests/ahci-mount-stress.cmds`, and `make test-ahci-mount-stress` for repeated mount/write/umount/remount validation plus `/proc/ahci_tune` telemetry capture.
...
- **libc portability phase 6 tranche-2 stdio landing completed 2026-04-02:** the stream-correctness half of tranche 2 is now in-tree. `include/stdio.h` gained `fpos_t`, seek/tell/fpos declarations, and `sscanf`/`vsscanf`; `user/stdio.c` now implements the seek/tell family plus a first-pass scanner for integer, character, string, scanset, and `%n` conversions; `fread` was fixed to honor `ungetc` and memory-backed streams; and the old placeholder `sscanf` stub was removed from `user/fmt.c`. Validation passed with a canonical `stdio.h` compile probe under `-Werror`, a sudo rebuild of `user/stdio.o`, `user/fmt.o`, and `aux.kern`, and a sudo dash-port rebuild with no new downstream failures.
...
- [x] Add long-run AHCI soak harness (`make test-ahci-mount-soak`)
...
- [x] Mount/unmount cycle succeeds repeatedly without controller lockup
