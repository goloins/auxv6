...
| USB subsystem | 46% | Initial PCI USB host-controller scaffold is in-tree (`UHCI/OHCI/EHCI/xHCI` classification, per-controller `init/ready/degraded` phase tracking, conservative BAR0 register sanity probes, `/proc/usb` observability, split HCD backends behind core ops dispatch, and backend lifecycle hooks for `probe/reset/halt/start` with explicit error-state reporting). All four backends now have real guarded control paths: EHCI and xHCI use MMIO `USBCMD`/`USBSTS` sequencing (xHCI adds `CNR` wait after reset); OHCI uses `HcCommandStatus.HCR` for reset and `HcControl.HCFS` field for `SUSPEND`/`OPERATIONAL` transitions; UHCI uses I/O-space `inw`/`outw` on `USBCMD`/`USBSTS` (RS bit + HCH flag) with HCRESET soft-reset. Per-controller lifecycle attempt/success/failure counters (`p/r/h/s` fields) are tracked in-state and emitted in `/proc/usb`. A minimal root-hub state scaffold (`present/port-count/change-bitmap`) is wired for follow-on enumeration work. Transfer rings/schedules, full root-hub emulation, endpoint state machine, and class drivers remain follow-on |
