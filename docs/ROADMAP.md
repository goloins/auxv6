...
| USB subsystem | 42% | Initial PCI USB host-controller scaffold is in-tree (`UHCI/OHCI/EHCI/xHCI` classification, per-controller `init/ready/degraded` phase tracking, conservative BAR0 register sanity probes, `/proc/usb` observability, split HCD backends behind core ops dispatch, and backend lifecycle hooks for `probe/reset/halt/start` with explicit error-state reporting). Both EHCI and xHCI now include real guarded control paths for `run/stop` and host-controller reset with bounded polling (`USBCMD`/`USBSTS` register sequences + `CNR` wait for xHCI). Per-controller lifecycle attempt/success/failure counters (`p/r/h/s` fields) are tracked in-state and emitted in `/proc/usb`. A minimal root-hub state scaffold (`present/port-count/change-bitmap`) is wired for follow-on enumeration work. Transfer rings/schedules, full root-hub emulation, endpoint state machine, and class drivers remain follow-on |
