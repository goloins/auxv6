...
#define AUDIO_CREATIVE_LIVE_MAX 4

static struct audio_pci_stub_softc sc[AUDIO_CREATIVE_LIVE_MAX];
static int count;

static const struct audio_pci_stub_match match_tbl[] = {
  { "creative-live", PCI_VENDOR_CREATIVE, 0x0002,
...
void
audio_creative_live_init(void)
{
  int i;
  BOOTDBG("audio/creative-live: probe\n");
  for(i = 0; i < pci_device_count() && count < AUDIO_CREATIVE_LIVE_MAX; i++){
