...
#define AUDIO_CREATIVE_XFI_MAX 4

static struct audio_pci_stub_softc sc[AUDIO_CREATIVE_XFI_MAX];
static int count;

static const struct audio_pci_stub_match match_tbl[] = {
  { "creative-xfi", PCI_VENDOR_CREATIVE, 0x0005,
...
void
audio_creative_xfi_init(void)
{
  int i;
  BOOTDBG("audio/creative-xfi: probe\n");
  for(i = 0; i < pci_device_count() && count < AUDIO_CREATIVE_XFI_MAX; i++){
