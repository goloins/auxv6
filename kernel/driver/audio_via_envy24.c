...
#define AUDIO_VIA_ENVY24_MAX 4

static struct audio_pci_stub_softc sc[AUDIO_VIA_ENVY24_MAX];
static int count;

static const struct audio_pci_stub_match match_tbl[] = {
  { "via-envy24", PCI_VENDOR_VIA, 0x1724,
...
void
audio_via_envy24_init(void)
{
  int i;
  BOOTDBG("audio/via-envy24: probe\n");
  for(i = 0; i < pci_device_count() && count < AUDIO_VIA_ENVY24_MAX; i++){
