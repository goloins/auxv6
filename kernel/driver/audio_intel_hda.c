...
#define AUDIO_INTEL_HDA_MAX 4

static struct audio_pci_stub_softc sc[AUDIO_INTEL_HDA_MAX];
static int count;

static const struct audio_pci_stub_match match_tbl[] = {
  { "intel-hda", PCI_VENDOR_INTEL, AUDIO_PCI_ANY_DEVICE,
...
void
audio_intel_hda_init(void)
{
  int i;
  BOOTDBG("audio/intel-hda: probe\n");
  for(i = 0; i < pci_device_count() && count < AUDIO_INTEL_HDA_MAX; i++){
