...
#define AUDIO_REALTEK_HDA_MAX 4

static struct audio_pci_stub_softc sc[AUDIO_REALTEK_HDA_MAX];
static int count;

static const struct audio_pci_stub_match match_tbl[] = {
  { "realtek-hda", PCI_VENDOR_REALTEK, AUDIO_PCI_ANY_DEVICE,
...
void
audio_realtek_hda_init(void)
{
  int i;
  BOOTDBG("audio/realtek-hda: probe\n");
  for(i = 0; i < pci_device_count() && count < AUDIO_REALTEK_HDA_MAX; i++){
