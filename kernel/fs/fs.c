...
#define DEFAULT_DIR_MODE (M_IRUSR | M_IWUSR | M_IXUSR | M_IRGRP | M_IXGRP | M_IROTH | M_IXOTH)
...
ialloc(uint dev, short type)
{
  ...
  for(inum = 1; inum < sb.ninodes; inum++){
    ...
    if(dip->type == 0){  // a free inode
      ...
      dip->type = type;
      dip->mode = inode_default_mode(type);
...
int
inode_on_dev(struct inode *ip, uint dev)
{
  return ip != 0 && ip->dev == dev;
}
...
static int
inode_allowed_bits(struct inode *ip, struct proc *p)
{
  int mode;
  ...
  mode = ip->mode;
  if(mode == 0)
    mode = inode_default_mode(ip->type);
...
int
iaccess(struct inode *ip, int mode)
{
  int allowed;

  allowed = inode_allowed_bits(ip, myproc());
  if((mode & IACC_READ) && !(allowed & 4))
    return -1;
  if((mode & IACC_WRITE) && !(allowed & 2))
    return -1;
  if((mode & IACC_EXEC) && !(allowed & 1))
    return -1;
  return 0;
}
...
// listed in block ip->addrs[INDIRECT_INDEX]. The remaining
// NDINDIRECT blocks are addressed through ip->addrs[DINDIRECT_INDEX].
...
itrunc(struct inode *ip)
{
  ...
  if(ip->addrs[DINDIRECT_INDEX]){
    ...
    brelse(bp);
    bfree(ip->dev, ip->addrs[DINDIRECT_INDEX]);
    ip->addrs[DINDIRECT_INDEX] = 0;
  }
