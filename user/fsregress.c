#include "types.h"
#include "stat.h"
#include "auxv6/user.h"
#include "fcntl.h"
#include "fs.h"

#define FSREG_BATCH 8
#define FSREG_PATH_MAX 128

struct scan_target {
  char *path;
  int required;
  int strict_stat;
};

static char *mnt_root = "/mnt";
static char mpath_pool[8][FSREG_PATH_MAX];
static int mpath_next;
static int fsreg_debug;

#define DBG(...) do { if(fsreg_debug) dprintf(1, __VA_ARGS__); } while(0)

static void
copy_name(char *dst, struct dirent *de)
{
  memmove(dst, de->name, DIRSIZ);
  dst[DIRSIZ] = 0;
}

static int
join_path(char *out, int outsz, char *base, char *name)
{
  int blen;
  int nlen;
  int need;
  int p;

  blen = strlen(base);
  nlen = strlen(name);
  need = blen + ((blen > 1) ? 1 : 0) + nlen + 1;
  if(need > outsz)
    return -1;

  memmove(out, base, blen);
  p = blen;
  if(blen > 1)
    out[p++] = '/';
  memmove(out + p, name, nlen);
  out[p + nlen] = 0;
  DBG("fsregress[d]: join_path base=%s name=%s -> %s\n", base, name, out);
  return 0;
}

static char*
mpath(char *name)
{
  char *out;

  out = mpath_pool[mpath_next];
  mpath_next = (mpath_next + 1) % 8;
  if(join_path(out, FSREG_PATH_MAX, mnt_root, name) < 0){
    dprintf(1, "fsregress: FAIL path too long %s/%s\n", mnt_root, name);
    exit(0);
  }
  return out;
}

static void
scan_dir(struct scan_target *t, int rounds, int *total_entries)
{
  int r;
  int saw_any;

  saw_any = 0;
  DBG("fsregress[d]: scan_dir path=%s rounds=%d required=%d strict=%d\n",
      t->path, rounds, t->required, t->strict_stat);
  for(r = 0; r < rounds; r++){
    int fd;
    struct stat st;

    fd = open(t->path, O_RDONLY);
    DBG("fsregress[d]: open(%s,O_RDONLY) -> %d\n", t->path, fd);
    if(fd < 0){
      if(!t->required){
        dprintf(1, "fsregress: skip %s (not available)\n", t->path);
        return;
      }
      dprintf(1, "fsregress: FAIL open %s\n", t->path);
      exit(0);
    }

    if(fstat(fd, &st) < 0 || st.st_type != T_DIR){
      dprintf(1, "fsregress: FAIL not a dir %s\n", t->path);
      close(fd);
      exit(0);
    }

    for(;;){
      struct dirent ents[FSREG_BATCH];
      int n;
      int i;

      n = getdents(fd, ents, FSREG_BATCH);
      if(n < 0){
        dprintf(1, "fsregress: FAIL getdents %s\n", t->path);
        close(fd);
        exit(0);
      }
      if(n == 0)
        break;

      for(i = 0; i < n; i++){
        char name[DIRSIZ + 1];

        if(ents[i].inum == 0)
          continue;
        copy_name(name, &ents[i]);
        if(name[0] == 0)
          continue;
        saw_any = 1;
        *total_entries = *total_entries + 1;

        if(t->strict_stat && strcmp(name, ".") != 0 && strcmp(name, "..") != 0){
          char full[FSREG_PATH_MAX];
          struct stat est;

          if(join_path(full, sizeof(full), t->path, name) < 0){
            dprintf(1, "fsregress: FAIL path too long %s/%s\n", t->path, name);
            close(fd);
            exit(0);
          }
          if(stat(full, &est) < 0){
            dprintf(1, "fsregress: FAIL stat %s\n", full);
            close(fd);
            exit(0);
          }
        }
      }
    }

    close(fd);
  }

  if(t->required && !saw_any){
    dprintf(1, "fsregress: FAIL empty required dir %s\n", t->path);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s rounds=%d\n", t->path, rounds);
}

static void
check_mnt_link_cycle(void)
{
  char *a;
  char *b;
  char *c;
  int fd;
  int ra;
  int rb;
  int rc;
  struct stat sa;
  struct stat sb;
  char buf[8];

  a = mpath(".fsra");
  b = mpath(".fsrb");
  c = mpath(".fsrc");
  DBG("fsregress[d]: link-cycle paths a=%s b=%s c=%s\n", a, b, c);

  fd = open(mnt_root, O_RDONLY);
  DBG("fsregress[d]: open(%s,O_RDONLY) -> %d\n", mnt_root, fd);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s link cycle (not available)\n", mnt_root);
    return;
  }
  close(fd);

  ra = unlink(a);
  rb = unlink(b);
  rc = unlink(c);
  DBG("fsregress[d]: preclean unlink a=%d b=%d c=%d\n", ra, rb, rc);

  fd = open(a, O_CREATE | O_WRONLY | O_TRUNC);
  DBG("fsregress[d]: open(%s,O_CREATE|O_WRONLY|O_TRUNC) -> %d\n", a, fd);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", a);
    exit(0);
  }
  if(write(fd, "hi\n", 3) != 3){
    dprintf(1, "fsregress: FAIL write %s\n", a);
    close(fd);
    exit(0);
  }
  close(fd);

  if(link(a, b) < 0){
    dprintf(1, "fsregress: FAIL link %s -> %s\n", a, b);
    exit(0);
  }

  if(stat(a, &sa) < 0 || stat(b, &sb) < 0){
    dprintf(1, "fsregress: FAIL stat link pair\n");
    exit(0);
  }
  if(sa.st_ino != sb.st_ino || sa.st_nlink < 2 || sb.st_nlink < 2){
    dprintf(1, "fsregress: FAIL bad link metadata\n");
    exit(0);
  }

  if(unlink(a) < 0){
    dprintf(1, "fsregress: FAIL unlink %s\n", a);
    exit(0);
  }

  fd = open(b, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL open survivor %s\n", b);
    exit(0);
  }
  if(read(fd, buf, 3) != 3){
    dprintf(1, "fsregress: FAIL read survivor %s\n", b);
    close(fd);
    exit(0);
  }
  close(fd);

  if(unlink(b) < 0){
    dprintf(1, "fsregress: FAIL final unlink %s\n", b);
    exit(0);
  }
  if(stat(b, &sb) >= 0){
    dprintf(1, "fsregress: FAIL stale path after unlink %s\n", b);
    exit(0);
  }

  fd = open(c, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL recreate after unlink %s\n", c);
    exit(0);
  }
  if(write(fd, "ok\n", 3) != 3){
    dprintf(1, "fsregress: FAIL write recreate %s\n", c);
    close(fd);
    exit(0);
  }
  close(fd);
  if(unlink(c) < 0){
    dprintf(1, "fsregress: FAIL cleanup %s\n", c);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s link cycle\n", mnt_root);
}

static void
check_mnt_rename_cycle(void)
{
  char *a;
  char *b;
  char buf[8];
  int fd;
  struct stat sa;
  struct stat sb;

  a = mpath(".fsrra");
  b = mpath(".fsrrb");
  DBG("fsregress[d]: rename-cycle paths a=%s b=%s\n", a, b);

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s rename cycle (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(a);
  unlink(b);

  fd = open(a, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", a);
    exit(0);
  }
  if(write(fd, "rn\n", 3) != 3){
    dprintf(1, "fsregress: FAIL write %s\n", a);
    close(fd);
    exit(0);
  }
  close(fd);

  fd = open(b, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", b);
    exit(0);
  }
  if(write(fd, "old\n", 4) != 4){
    dprintf(1, "fsregress: FAIL write %s\n", b);
    close(fd);
    exit(0);
  }
  close(fd);

  if(rename(a, b) < 0){
    dprintf(1, "fsregress: FAIL rename %s -> %s\n", a, b);
    exit(0);
  }
  if(stat(a, &sa) >= 0){
    dprintf(1, "fsregress: FAIL old path still exists %s\n", a);
    exit(0);
  }
  if(stat(b, &sb) < 0){
    dprintf(1, "fsregress: FAIL missing new path %s\n", b);
    exit(0);
  }
  fd = open(b, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL open overwritten path %s\n", b);
    exit(0);
  }
  if(read(fd, buf, 3) != 3 || buf[0] != 'r' || buf[1] != 'n' || buf[2] != '\n'){
    dprintf(1, "fsregress: FAIL overwrite content %s\n", b);
    close(fd);
    exit(0);
  }
  close(fd);

  if(rename(b, b) < 0){
    dprintf(1, "fsregress: FAIL rename no-op %s\n", b);
    exit(0);
  }
  if(unlink(b) < 0){
    dprintf(1, "fsregress: FAIL cleanup %s\n", b);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s rename cycle\n", mnt_root);
}

static void
check_mnt_crossdir_rename_cycle(void)
{
  char *d1;
  char *d2;
  char *src;
  char *dst;
  char buf[8];
  int fd;
  struct stat st;

  d1 = mpath(".fsrd1");
  d2 = mpath(".fsrd2");
  src = mpath(".fsrd1/src");
  dst = mpath(".fsrd2/dst");
  DBG("fsregress[d]: crossdir paths d1=%s d2=%s src=%s dst=%s\n", d1, d2, src, dst);

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s crossdir rename (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(src);
  unlink(dst);
  rmdir(d1);
  rmdir(d2);

  if(mkdir(d1) < 0 || mkdir(d2) < 0){
    dprintf(1, "fsregress: FAIL mkdir crossdir roots\n");
    exit(0);
  }

  fd = open(src, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", src);
    exit(0);
  }
  if(write(fd, "xy\n", 3) != 3){
    dprintf(1, "fsregress: FAIL write %s\n", src);
    close(fd);
    exit(0);
  }
  close(fd);

  fd = open(dst, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", dst);
    exit(0);
  }
  if(write(fd, "old\n", 4) != 4){
    dprintf(1, "fsregress: FAIL write %s\n", dst);
    close(fd);
    exit(0);
  }
  close(fd);

  if(rename(src, dst) < 0){
    dprintf(1, "fsregress: FAIL crossdir rename %s -> %s\n", src, dst);
    exit(0);
  }
  if(stat(src, &st) >= 0){
    dprintf(1, "fsregress: FAIL stale src after rename %s\n", src);
    exit(0);
  }
  if(stat(dst, &st) < 0){
    dprintf(1, "fsregress: FAIL missing dst after rename %s\n", dst);
    exit(0);
  }

  fd = open(dst, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL open dst %s\n", dst);
    exit(0);
  }
  if(read(fd, buf, 3) != 3 || buf[0] != 'x' || buf[1] != 'y' || buf[2] != '\n'){
    dprintf(1, "fsregress: FAIL dst content %s\n", dst);
    close(fd);
    exit(0);
  }
  close(fd);

  if(unlink(dst) < 0 || rmdir(d1) < 0 || rmdir(d2) < 0){
    dprintf(1, "fsregress: FAIL crossdir cleanup\n");
    exit(0);
  }

  dprintf(1, "fsregress: ok %s crossdir rename\n", mnt_root);
}

static void
check_mnt_dir_rename_cycle(void)
{
  char *d1;
  char *d2;
  char *src;
  char *dst;
  char *sub;
  char *bad;
  int fd;
  struct stat st;

  d1 = mpath(".fsdd1");
  d2 = mpath(".fsdd2");
  src = mpath(".fsdd1/src");
  dst = mpath(".fsdd2/dst");
  sub = mpath(".fsdd2/dst/sub");
  bad = mpath(".fsdd2/dst/sub/oops");
  DBG("fsregress[d]: dir-rename paths d1=%s d2=%s src=%s dst=%s sub=%s bad=%s\n",
      d1, d2, src, dst, sub, bad);

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s dir rename (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(sub);
  rmdir(sub);
  rmdir(dst);
  unlink(src);
  rmdir(d1);
  rmdir(d2);

  if(mkdir(d1) < 0 || mkdir(d2) < 0 || mkdir(src) < 0){
    dprintf(1, "fsregress: FAIL mkdir dir rename roots\n");
    exit(0);
  }

  if(rename(src, dst) < 0){
    dprintf(1, "fsregress: FAIL dir rename %s -> %s\n", src, dst);
    exit(0);
  }
  if(stat(src, &st) >= 0){
    dprintf(1, "fsregress: FAIL stale src dir after rename %s\n", src);
    exit(0);
  }
  if(stat(dst, &st) < 0 || st.st_type != T_DIR){
    dprintf(1, "fsregress: FAIL missing dst dir after rename %s\n", dst);
    exit(0);
  }

  if(mkdir(sub) < 0){
    dprintf(1, "fsregress: FAIL mkdir subdir under moved dir %s\n", sub);
    exit(0);
  }

  if(rename(d2, bad) >= 0){
    dprintf(1, "fsregress: FAIL subtree move unexpectedly succeeded\n");
    exit(0);
  }

  if(rmdir(sub) < 0 || rmdir(dst) < 0 || rmdir(d1) < 0 || rmdir(d2) < 0){
    dprintf(1, "fsregress: FAIL dir rename cleanup\n");
    exit(0);
  }

  dprintf(1, "fsregress: ok %s dir rename cycle\n", mnt_root);
}

static void
check_mnt_indirect_write_cycle(void)
{
  char *p;
  char buf[1024];
  int fd;
  int i;
  int j;
  struct stat st;

  p = mpath(".fsbig");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s indirect write (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(p);
  fd = open(p, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", p);
    exit(0);
  }

  for(i = 0; i < 20; i++){
    char v;

    v = 'A' + (i % 26);
    for(j = 0; j < sizeof(buf); j++)
      buf[j] = v;
    if(write(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL write block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != 20 * 1024){
    dprintf(1, "fsregress: FAIL size after indirect write %s\n", p);
    exit(0);
  }

  fd = open(p, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL reopen %s\n", p);
    exit(0);
  }

  for(i = 0; i < 20; i++){
    char v;

    v = 'A' + (i % 26);
    if(read(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL read block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
    for(j = 0; j < sizeof(buf); j++){
      if(buf[j] != v){
        dprintf(1, "fsregress: FAIL data mismatch block %d byte %d %s\n", i, j, p);
        close(fd);
        exit(0);
      }
    }
  }
  close(fd);

  if(unlink(p) < 0){
    dprintf(1, "fsregress: FAIL cleanup %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s indirect write cycle\n", mnt_root);
}

static void
check_mnt_indirect_edge_cycle(void)
{
  char *p;
  char buf[1024];
  int fd;
  int i;
  int j;
  struct stat st;

  p = mpath(".fsedge");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s indirect edge (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(p);
  fd = open(p, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", p);
    exit(0);
  }

  // Write exactly direct-block coverage first (12 * 1024), then one more block.
  for(i = 0; i < 13; i++){
    char v;

    v = 'a' + (i % 26);
    for(j = 0; j < sizeof(buf); j++)
      buf[j] = v;
    if(write(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL boundary write block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != 13 * 1024){
    dprintf(1, "fsregress: FAIL boundary size %s\n", p);
    exit(0);
  }

  fd = open(p, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL boundary reopen %s\n", p);
    exit(0);
  }
  for(i = 0; i < 13; i++){
    char v;

    v = 'a' + (i % 26);
    if(read(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL boundary read block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
    for(j = 0; j < sizeof(buf); j++){
      if(buf[j] != v){
        dprintf(1, "fsregress: FAIL boundary data mismatch block %d byte %d %s\n", i, j, p);
        close(fd);
        exit(0);
      }
    }
  }
  close(fd);

  // Truncate file after indirect usage and confirm size and new contents reset correctly.
  fd = open(p, O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL boundary trunc open %s\n", p);
    exit(0);
  }
  for(j = 0; j < sizeof(buf); j++)
    buf[j] = 'Z';
  if(write(fd, buf, sizeof(buf)) != sizeof(buf)){
    dprintf(1, "fsregress: FAIL boundary rewrite %s\n", p);
    close(fd);
    exit(0);
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != 1024){
    dprintf(1, "fsregress: FAIL boundary trunc size %s\n", p);
    exit(0);
  }

  fd = open(p, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL boundary final reopen %s\n", p);
    exit(0);
  }
  if(read(fd, buf, sizeof(buf)) != sizeof(buf)){
    dprintf(1, "fsregress: FAIL boundary final read %s\n", p);
    close(fd);
    exit(0);
  }
  close(fd);
  for(j = 0; j < sizeof(buf); j++){
    if(buf[j] != 'Z'){
      dprintf(1, "fsregress: FAIL boundary final data mismatch byte %d %s\n", j, p);
      exit(0);
    }
  }

  if(unlink(p) < 0){
    dprintf(1, "fsregress: FAIL boundary cleanup %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s indirect edge cycle\n", mnt_root);
}

static void
check_mnt_append_boundary_cycle(void)
{
  char *p;
  char buf[1024];
  int fd;
  int i;
  int j;
  struct stat st;

  p = mpath(".fsapp");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s append boundary (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(p);
  fd = open(p, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", p);
    exit(0);
  }
  for(j = 0; j < sizeof(buf); j++)
    buf[j] = 'Q';
  for(i = 0; i < 12; i++){
    if(write(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL append base write block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
  }
  close(fd);

  fd = open(p, O_WRONLY | O_APPEND);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL append open %s\n", p);
    exit(0);
  }
  for(j = 0; j < sizeof(buf); j++)
    buf[j] = 'R';
  if(write(fd, buf, sizeof(buf)) != sizeof(buf) ||
     write(fd, buf, sizeof(buf)) != sizeof(buf)){
    dprintf(1, "fsregress: FAIL append writes %s\n", p);
    close(fd);
    exit(0);
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != 14 * 1024){
    dprintf(1, "fsregress: FAIL append size %s\n", p);
    exit(0);
  }

  fd = open(p, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL append read open %s\n", p);
    exit(0);
  }
  for(i = 0; i < 14; i++){
    char v;

    v = (i < 12) ? 'Q' : 'R';
    if(read(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL append read block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
    for(j = 0; j < sizeof(buf); j++){
      if(buf[j] != v){
        dprintf(1, "fsregress: FAIL append data mismatch block %d byte %d %s\n", i, j, p);
        close(fd);
        exit(0);
      }
    }
  }
  close(fd);

  if(unlink(p) < 0){
    dprintf(1, "fsregress: FAIL append cleanup %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s append boundary cycle\n", mnt_root);
}

static void
check_mnt_write_fail_rollback_cycle(void)
{
  char *p;
  char buf[1024];
  char big[2048];
  int fd;
  int i;
  struct stat st;

  p = mpath(".fsfail");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s write fail rollback (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(p);
  fd = open(p, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", p);
    exit(0);
  }

  for(i = 0; i < sizeof(buf); i++)
    buf[i] = 'W';
  for(i = 0; i < sizeof(big); i++)
    big[i] = 'W';

  // Fail block allocation on the 2nd allocation call in this sequence.
  if(ext2fail(1, 1) < 0){
    dprintf(1, "fsregress: skip %s write fail rollback (ext2fail unsupported)\n", mnt_root);
    close(fd);
    unlink(p);
    return;
  }
  if(write(fd, big, sizeof(big)) >= 0){
    dprintf(1, "fsregress: skip %s write fail rollback (no injected failure)\n", mnt_root);
    ext2fail(0, 0);
    close(fd);
    unlink(p);
    return;
  }
  if(ext2fail(0, 0) < 0){
    dprintf(1, "fsregress: FAIL ext2fail disarm\n");
    close(fd);
    exit(0);
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != 0){
    dprintf(1, "fsregress: FAIL rollback size %s\n", p);
    exit(0);
  }

  fd = open(p, O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL rollback reopen %s\n", p);
    exit(0);
  }
  if(write(fd, buf, sizeof(buf)) != sizeof(buf)){
    dprintf(1, "fsregress: FAIL rollback rewrite %s\n", p);
    close(fd);
    exit(0);
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != sizeof(buf)){
    dprintf(1, "fsregress: FAIL rollback final size %s\n", p);
    exit(0);
  }

  if(unlink(p) < 0){
    dprintf(1, "fsregress: FAIL rollback cleanup %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s write fail rollback cycle\n", mnt_root);
}

static void
check_mnt_unlink_inodefail_cycle(void)
{
  char *p;
  char buf[8];
  int fd;
  struct stat st;

  p = mpath(".fsiunlk");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s unlink inodefail (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(p);
  fd = open(p, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", p);
    exit(0);
  }
  if(write(fd, "ok\n", 3) != 3){
    dprintf(1, "fsregress: FAIL seed write %s\n", p);
    close(fd);
    exit(0);
  }
  close(fd);

  // Force first inode write in unlink path to fail (parent inode update).
  if(ext2fail(2, 0) < 0){
    dprintf(1, "fsregress: skip %s unlink inodefail (ext2fail unsupported)\n", mnt_root);
    unlink(p);
    return;
  }
  if(unlink(p) >= 0){
    dprintf(1, "fsregress: skip %s unlink inodefail (no injected failure)\n", mnt_root);
    ext2fail(0, 0);
    return;
  }
  if(ext2fail(0, 0) < 0){
    dprintf(1, "fsregress: FAIL ext2fail inode disarm\n");
    exit(0);
  }

  if(stat(p, &st) < 0){
    dprintf(1, "fsregress: FAIL missing file after failed unlink %s\n", p);
    exit(0);
  }
  fd = open(p, O_RDONLY);
  if(fd < 0 || read(fd, buf, 3) != 3){
    dprintf(1, "fsregress: FAIL read survivor after failed unlink %s\n", p);
    if(fd >= 0)
      close(fd);
    exit(0);
  }
  close(fd);

  if(unlink(p) < 0){
    dprintf(1, "fsregress: FAIL final unlink %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s unlink inodefail cycle\n", mnt_root);
}

static void
check_mnt_mkdir_allocfail_cycle(void)
{
  char *p;
  int fd;
  struct stat st;

  p = mpath(".fsnospace");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s mkdir allocfail (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(p);

  // Force allocation failure in ext2_alloc_block() for mkdir data block allocation.
  if(ext2fail(1, 0) < 0){
    dprintf(1, "fsregress: skip %s mkdir allocfail (ext2fail unsupported)\n", mnt_root);
    return;
  }
  if(mkdir(p) >= 0){
    dprintf(1, "fsregress: skip %s mkdir allocfail (no injected failure)\n", mnt_root);
    ext2fail(0, 0);
    rmdir(p);
    return;
  }
  if(ext2fail(0, 0) < 0){
    dprintf(1, "fsregress: FAIL ext2fail alloc disarm\n");
    exit(0);
  }

  if(stat(p, &st) >= 0){
    dprintf(1, "fsregress: FAIL stale dir after failed mkdir %s\n", p);
    exit(0);
  }

  if(mkdir(p) < 0){
    dprintf(1, "fsregress: FAIL mkdir after allocfail %s\n", p);
    exit(0);
  }
  if(rmdir(p) < 0){
    dprintf(1, "fsregress: FAIL cleanup mkdir allocfail %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s mkdir allocfail cycle\n", mnt_root);
}

static void
check_mnt_indirect_capacity_cycle(void)
{
  char *p;
  char buf[1024];
  int fd;
  int i;
  int j;
  struct stat st;

  // For ext2 image built with 1KiB blocks, single-indirect coverage is 12 + 256 blocks.
  int max_blocks;

  p = mpath(".fscap");
  max_blocks = 12 + 256;

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s indirect capacity (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(p);
  fd = open(p, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", p);
    exit(0);
  }

  for(j = 0; j < sizeof(buf); j++)
    buf[j] = 'C';
  for(i = 0; i < max_blocks; i++){
    if(write(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL capacity base write block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
  }

  if(write(fd, buf, sizeof(buf)) >= 0){
    dprintf(1, "fsregress: FAIL expected capacity write failure %s\n", p);
    close(fd);
    exit(0);
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != max_blocks * 1024){
    dprintf(1, "fsregress: FAIL capacity size %s\n", p);
    exit(0);
  }

  fd = open(p, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL capacity reopen %s\n", p);
    exit(0);
  }
  for(i = 0; i < max_blocks; i++){
    if(read(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL capacity read block %d %s\n", i, p);
      close(fd);
      exit(0);
    }
    for(j = 0; j < sizeof(buf); j++){
      if(buf[j] != 'C'){
        dprintf(1, "fsregress: FAIL capacity data mismatch block %d byte %d %s\n", i, j, p);
        close(fd);
        exit(0);
      }
    }
  }
  close(fd);

  if(unlink(p) < 0){
    dprintf(1, "fsregress: FAIL capacity cleanup %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s indirect capacity cycle\n", mnt_root);
}

static void
check_mnt_churn_cycle(void)
{
  char *base;
  char *lnk;
  char *mv;
  int i;
  int fd;
  struct stat s1;
  struct stat s2;
  char buf[8];

  base = mpath(".fschurn_a");
  lnk = mpath(".fschurn_b");
  mv = mpath(".fschurn_c");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s churn (not available)\n", mnt_root);
    return;
  }
  close(fd);

  for(i = 0; i < 100; i++){
    unlink(base);
    unlink(lnk);
    unlink(mv);

    fd = open(base, O_CREATE | O_WRONLY | O_TRUNC);
    if(fd < 0){
      dprintf(1, "fsregress: FAIL churn create iter=%d\n", i);
      exit(0);
    }
    if(write(fd, "ch\n", 3) != 3){
      dprintf(1, "fsregress: FAIL churn write iter=%d\n", i);
      close(fd);
      exit(0);
    }
    close(fd);

    if(link(base, lnk) < 0){
      dprintf(1, "fsregress: FAIL churn link iter=%d\n", i);
      exit(0);
    }
    if(rename(lnk, mv) < 0){
      dprintf(1, "fsregress: FAIL churn rename iter=%d\n", i);
      exit(0);
    }

    if(stat(base, &s1) < 0 || stat(mv, &s2) < 0){
      dprintf(1, "fsregress: FAIL churn stat iter=%d\n", i);
      exit(0);
    }
    if(s1.st_ino != s2.st_ino || s1.st_nlink < 2){
      dprintf(1, "fsregress: FAIL churn metadata iter=%d\n", i);
      exit(0);
    }

    fd = open(mv, O_RDONLY);
    if(fd < 0 || read(fd, buf, 3) != 3){
      dprintf(1, "fsregress: FAIL churn read iter=%d\n", i);
      if(fd >= 0)
        close(fd);
      exit(0);
    }
    close(fd);

    if(unlink(base) < 0 || unlink(mv) < 0){
      dprintf(1, "fsregress: FAIL churn cleanup iter=%d\n", i);
      exit(0);
    }
  }

  dprintf(1, "fsregress: ok %s churn cycle\n", mnt_root);
}

static void
check_mnt_generic_fsfault_cycle(void)
{
  char *p;
  char buf[1024];
  char big[2048];
  int fd;
  int i;
  int dev;
  struct stat st;

  p = mpath(".fsgen");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s generic fsfault (not available)\n", mnt_root);
    return;
  }
  close(fd);

  if(stat(mnt_root, &st) < 0){
    dprintf(1, "fsregress: skip %s generic fsfault (no stat)\n", mnt_root);
    return;
  }
  dev = st.st_dev;

  unlink(p);
  fd = open(p, O_CREATE | O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL create %s\n", p);
    exit(0);
  }

  for(i = 0; i < sizeof(buf); i++)
    buf[i] = 'G';
  for(i = 0; i < sizeof(big); i++)
    big[i] = 'G';

  if(fsfault(dev, 1, 1) < 0){
    dprintf(1, "fsregress: skip %s generic fsfault (unsupported)\n", mnt_root);
    close(fd);
    unlink(p);
    return;
  }
  if(write(fd, big, sizeof(big)) >= 0){
    dprintf(1, "fsregress: skip %s generic fsfault (no injected failure)\n", mnt_root);
    fsfault(dev, 0, 0);
    close(fd);
    unlink(p);
    return;
  }
  if(fsfault(dev, 0, 0) < 0){
    dprintf(1, "fsregress: FAIL fsfault disarm\n");
    close(fd);
    exit(0);
  }
  close(fd);

  if(stat(p, &st) < 0 || st.st_size != 0){
    dprintf(1, "fsregress: FAIL generic fsfault rollback size %s\n", p);
    exit(0);
  }

  fd = open(p, O_WRONLY | O_TRUNC);
  if(fd < 0){
    dprintf(1, "fsregress: FAIL generic fsfault reopen %s\n", p);
    exit(0);
  }
  if(write(fd, buf, sizeof(buf)) != sizeof(buf)){
    dprintf(1, "fsregress: FAIL generic fsfault rewrite %s\n", p);
    close(fd);
    exit(0);
  }
  close(fd);

  if(unlink(p) < 0){
    dprintf(1, "fsregress: FAIL generic fsfault cleanup %s\n", p);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s generic fsfault cycle\n", mnt_root);
}

static void
check_mnt_devnode_cycle(void)
{
  char *cpath;
  char *bpath;
  int dev;
  int fd;
  char buf[32];
  struct stat st;

  cpath = mpath(".fsrdevc");
  bpath = mpath(".fsrdevb");

  fd = open(mnt_root, O_RDONLY);
  if(fd < 0){
    dprintf(1, "fsregress: skip %s devnode cycle (not available)\n", mnt_root);
    return;
  }
  close(fd);

  unlink(cpath);
  unlink(bpath);

  if(mknod(cpath, M_IFCHR, 1, 7) < 0){
    dprintf(1, "fsregress: FAIL mknod %s\n", cpath);
    exit(0);
  }

  if(stat(cpath, &st) < 0){
    dprintf(1, "fsregress: FAIL stat dev %s\n", cpath);
    exit(0);
  }
  if(st.st_type != T_DEV){
    dprintf(1, "fsregress: FAIL wrong type dev %s\n", cpath);
    exit(0);
  }
  if((st.st_mode & M_IFMT) != M_IFCHR || st.st_major != 1 || st.st_minor != 7){
    dprintf(1, "fsregress: FAIL bad dev ids major=%d minor=%d\n", st.st_major, st.st_minor);
    exit(0);
  }

  dev = -1;
  for(fd = 0; fd < HD_DISK_UNITS; fd++){
    if(devblocks(HD_DISK_DEV(fd)) > 0){
      dev = HD_DISK_DEV(fd);
      break;
    }
  }
  if(dev >= 0){
    if(mknod(bpath, M_IFBLK, 2, dev) < 0){
      dprintf(1, "fsregress: FAIL block mknod %s\n", bpath);
      exit(0);
    }
    if(stat(bpath, &st) < 0){
      dprintf(1, "fsregress: FAIL stat block dev %s\n", bpath);
      exit(0);
    }
    if(st.st_type != T_DEV || (st.st_mode & M_IFMT) != M_IFBLK || st.st_major != 2 || st.st_minor != dev){
      dprintf(1, "fsregress: FAIL bad block dev metadata major=%d minor=%d mode=%x\n", st.st_major, st.st_minor, st.st_mode);
      exit(0);
    }
    fd = open(bpath, O_RDONLY);
    if(fd < 0){
      dprintf(1, "fsregress: FAIL open block dev %s\n", bpath);
      exit(0);
    }
    if(read(fd, buf, sizeof(buf)) != sizeof(buf)){
      dprintf(1, "fsregress: FAIL read block dev %s\n", bpath);
      close(fd);
      exit(0);
    }
    close(fd);
  }

  if(unlink(cpath) < 0){
    dprintf(1, "fsregress: FAIL cleanup dev %s\n", cpath);
    exit(0);
  }
  if(dev >= 0 && unlink(bpath) < 0){
    dprintf(1, "fsregress: FAIL cleanup block dev %s\n", bpath);
    exit(0);
  }

  dprintf(1, "fsregress: ok %s devnode cycle\n", mnt_root);
}

int
main(int argc, char *argv[])
{
  struct scan_target targets[3];
  char *scan_path;
  int rounds;
  int total_entries;
  int i;
  int argi;
  int mount_set;
  int rounds_set;

  rounds = 20;
  scan_path = mnt_root;
  argi = 1;
  mount_set = 0;
  rounds_set = 0;
  fsreg_debug = 0;

  while(argi < argc){
    if(strcmp(argv[argi], "-d") == 0){
      fsreg_debug = 1;
      argi++;
      continue;
    }
    if(argv[argi][0] == '/' && !mount_set){
      scan_path = argv[argi];
      mount_set = 1;
      argi++;
      continue;
    }
    if(!rounds_set){
      rounds = atoi(argv[argi]);
      rounds_set = 1;
      argi++;
      continue;
    }
    dprintf(1, "usage: fsregress [-d] [mountpoint] [rounds]\n");
    exit(0);
  }

  DBG("fsregress[d]: parsed mount=%s rounds=%d debug=%d\n", scan_path, rounds, fsreg_debug);

  mnt_root = scan_path;

  targets[0].path = "/";
  targets[0].required = 1;
  targets[0].strict_stat = 1;
  targets[1].path = "/proc";
  targets[1].required = 1;
  targets[1].strict_stat = 0;
  targets[2].path = mnt_root;
  targets[2].required = 1;
  targets[2].strict_stat = 1;

  if(rounds <= 0)
    rounds = 1;

  total_entries = 0;
  dprintf(1, "fsregress: start rounds=%d\n", rounds);

  for(i = 0; i < sizeof(targets)/sizeof(targets[0]); i++)
    scan_dir(&targets[i], rounds, &total_entries);

  check_mnt_link_cycle();
  check_mnt_rename_cycle();
  check_mnt_crossdir_rename_cycle();
  check_mnt_dir_rename_cycle();
  check_mnt_indirect_write_cycle();
  check_mnt_indirect_edge_cycle();
  check_mnt_append_boundary_cycle();
  check_mnt_write_fail_rollback_cycle();
  check_mnt_unlink_inodefail_cycle();
  check_mnt_mkdir_allocfail_cycle();
  check_mnt_indirect_capacity_cycle();
  check_mnt_churn_cycle();
  check_mnt_generic_fsfault_cycle();
  check_mnt_devnode_cycle();

  dprintf(1, "fsregress: PASS entries=%d\n", total_entries);
  exit(0);
}
